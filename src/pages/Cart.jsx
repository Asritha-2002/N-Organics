...
const getBannerEligibleItemIds = (banner, items) => {
  ...
  for (const item of items) {
    ...
    if (banner.appliesTo === "all") {
      ...
      banner.appliesTo === "product" ||
      banner.appliesTo === "products"
    ) {
...
const CartItemCard = ({
  ...
  const hasAttrs = Object.values(item.attributes || {}).some(
    (v) => v && v !== "",
  );
  const discountPct =
          ...
          {hasAttrs && (
            <div className="flex flex-wrap gap-1.5">
              {item.attributes.size && (
                <span className="text-[10px] bg-[#457358]/8 text-[#457358] border border-[#457358]/20 px-2 py-0.5 rounded-full font-medium">
                  {item.attributes.size}
                </span>
              )}
            ...
            {!isUnavailable && (
              <div className="flex items-center gap-0 border border-[#e7dfd4] rounded-xl overflow-hidden">
                ...
                <button
                  ...
                  disabled={updating || item.quantity >= item.stockQty}
...
const VoucherSection = ({
  ...
  <span className="font-mono text-xs font-black text-emerald-700 tracking-widest">
    {v.code}
  </span>
  <span
    className={cn(
...
{/* After the description line in eligibleVouchers.map(...) */}
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="font-mono text-xs font-black text-gray-500 tracking-widest">
                            {v.code}
                          </span>
                        </div>
...
const OrderSummary = ({ pricing, onCheckout, loading, appliedVoucher }) => {
  const rows = [
    ...
    {
      label: "MRP Savings",
      value: -pricing.mrpSavings,
      show: pricing.mrpSavings > 0,
      color: "text-emerald-600",
      prefix: "-",
    },
    {
      label: "Banner Discount",
      value: -pricing.bannerDiscount,
      show: pricing.bannerDiscount > 0,
      color: "text-emerald-600",
      prefix: "-",
    },
    {
      ...
      show: pricing.voucherDiscount > 0,
      color: "text-emerald-600",
      prefix: "-",
    },
    {
  ...
  return (
    <div className="bg-white border border-[#e7dfd4] rounded-2xl overflow-hidden">
      <div className="px-5 py-4 border-b border-[#e7dfd4]">
        <h3 className="text-sm font-bold text-[#143c2f] uppercase tracking-wider">
          Order Summary
        </h3>
      </div>
      ...
      {row.customValue
        ...
        <div className="border-t border-dashed border-[#e7dfd4] pt-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xl font-black text-[#457358]">
              {fmt(pricing.total)}
            </span>
          </div>
...
const EmptyCart = () => (
  ...
  >
    ...
    <h2 className="text-xl font-bold text-[#143c2f] mb-2">
      Your cart is empty
    </h2>
    <p className="text-sm text-gray-400 mb-6 max-w-xs">
      Looks like you haven't added anything yet. Start exploring our skincare
      collection.
    </p>
    <Link
      to="/shop"
...
export default function Cart() {
  ...
  const handleBuyNowFromCart = async (item) => {
  try {
    const token = localStorage.getItem("token");
    ...
    toast.success(
      response?.data?.message || "Proceeding to checkout"
    );

    navigate("/checkout");
  } catch (error) {
    console.error("Buy now from cart error:", error);
  ...
  // ── Fetch cart ─────────────────────────────────────────────────────────────
  // ── Fetch cart (Supports Authenticated AND Guest Views) ─────────────────
  const fetchCart = useCallback(async () => {
    if (token) {
      ...
      // --- GUEST ROUTE (BATCH FETCH ALL AT ONCE) ---
      try {
        ...
        if (localCart.length === 0) {
          setCartData({ 
            items: [], 
            pricing: { subtotal: 0, mrpSavings: 0, bannerDiscount: 0, voucherDiscount: 0, total: 0, totalDiscount: 0 } 
          });
          setLoading(false);
          return;
        }

        // 1. Prepare payload from localStorage
        const itemsPayload = localCart.map(item => ({
        ...
        // 2. Make the single POST request to your new route
        const res = await fetch(`${BASE_URL}/bulk-details`, {
          method: "POST",
          headers: {
        ...
        // 3. Set the full data object directly into state (No mapping needed on frontend!)
        setCartData(json.data);
  ...
  const handleRemove = async (item) => {
    ...
    if (token) {
      ...
      // Guest local storage filter handler
      const localCart = JSON.parse(localStorage.getItem("guestCart")) || [];
    ...
    if (token) {
      try {
        ...
        if (!res.ok || !json.success) {
          ...
          if (json.maxAllowed) {
            const clampRes = await fetch(
              `${BASE_URL}/cart/quantity/${cartItemId}`,
              {
                method: "PATCH",
                headers: {
                  "Content-Type": "application/json",
      ...
      // Guest local modifications
      const localCart = JSON.parse(localStorage.getItem("guestCart")) || [];
      const targetItem = localCart.find(
        (i) =>
          i.productId === cartItem.productId &&
          i.variantIndex === cartItem.variantIndex,
      );

      if (targetItem) {
        if (newQty > cartItem.stockQty) {
          toast.error(`Only ${cartItem.stockQty} items available in stock`);
          setUpdatingId(null);
          return;
        }
  ...
  const handleApplyVoucher = async (code) => {
    try {
      ...
      if (!json.success) throw new Error(json.message);
      const appliedMsg =
  json.data?.discountType === "complimentary"
    ? json.message  // uses the "🎁 Complimentary gift added: 1x Rose Cream" message from backend
...
const handleCheckout = async () => {
  if (!token) {
    toast("Please sign in to finalize checkout", { icon: "🔒" });
  ...
  if (!defaultAddress) {
    ...
    setTimeout(() => navigate("/account/addresses"), 2600);
    return;
  }

  const unavailableItems = cartData?.items?.filter((i) => !i.isAvailable) || [];
  ...
  navigate("/checkout/payment", {
    state: {
      buyNowData: {
        item:          cartData.items[0],          // for sidebar preview (first item)
        pricing:       cartData.pricing,
        appliedVoucher: cartData.appliedVoucher || null,
        bestBanner:    cartData.bestBanner || null,
        items:         cartData.items,             // all items for cart order
      },
      address:         defaultAddress,
      isBuyNow:        false,
      isCartCheckout:  true,                       // ← key flag
    },
  });
  ...
  const fullBestBanner =
    cartData?.allApplicableBanners?.find(
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <div>
                            <p className="text-xs text-gray-400 uppercase tracking-wider font-bold">
                              {defaultAddress
                                ? `Deliver To ${defaultAddress.type}`
                                : "Delivery Address"}
                            </p>
                    ...
                    <AnimatePresence>
                      {availableItems.map((item) => (
                        <CartItemCard
                          ...
                          isBannerEligible={bannerEligibleIds.has(
                            String(item._id),
                          )}
                        />
                      ))}
                      ...
                      >
                        <div>
                          <p
                            className={cn(
                              "font-semibold",
                              i === 0 ? "text-[#457358]" : "text-gray-600",
                            )}
                          >
                            {b.title}
                            {i === 0 && (
                              <span className="ml-1.5 text-[9px] bg-[#457358] text-white px-1.5 py-0.5 rounded-full font-bold">
                                BEST
                              </span>
                            )}
                          </p>
