...
import {
  ...
  X, Image as ImageIcon, Leaf, Box, Info, Check,
  FolderOpen, Layers,Video
} from "lucide-react";
...
const TAG_COLORS     = {
  bestseller: { active: "bg-amber-500 text-white border-amber-500",   idle: "bg-white text-amber-700 border-amber-300 hover:bg-amber-50"  },
  new:        { active: "bg-sky-500 text-white border-sky-500",       idle: "bg-white text-sky-700 border-sky-300 hover:bg-sky-50"        },
  combo:      { active: "bg-violet-500 text-white border-violet-500", idle: "bg-white text-violet-700 border-violet-300 hover:bg-violet-50"},
  limited:    { active: "bg-rose-500 text-white border-rose-500",     idle: "bg-white text-rose-700 border-rose-300 hover:bg-rose-50"     },
};
const SKIN_TYPES     = ["all","dry","oily","combination","sensitive","normal","mature"];
const SKIN_CONCERNS  = ["acne","anti-aging","brightening","dark-spots","dryness","dullness","hyperpigmentation","pores","redness","sensitivity","uneven-texture","wrinkles"];
const CLAIMS         = ["vegan","cruelty-free","paraben-free","sulfate-free","fragrance-free","alcohol-free","dermatologist-tested","hypoallergenic","organic-certified","natural","reef-safe","non-comedogenic","gluten-free"];
...
const Toggle = ({ checked, onChange, label }) => (
  <label className="flex items-center gap-3 cursor-pointer select-none">
    <div onClick={() => onChange(!checked)}
      className={cn("relative w-10 h-5 rounded-full transition-colors duration-200", checked ? "bg-emerald-500" : "bg-gray-200")}>
    ...
    {options.map((opt) => {
      ...
      return (
        <button key={opt} type="button"
          ...
          className={cn("px-3 py-1.5 rounded-xl text-xs font-medium border capitalize transition",
            active ? "bg-emerald-600 text-white border-emerald-600" : "bg-gray-50 text-gray-600 border-gray-200 hover:border-emerald-300")}>
          {opt}
        </button>
      );
    })}
...
const SingleImageUpload = ({ file, onAdd, onRemove, label, hint }) => {
  ...
  const handleFile = (f) => {
    ...
    onAdd({ id: Date.now(), file: f, preview: URL.createObjectURL(f), existing: false });
  };
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {!file ? (
        ...
        <div className="relative rounded-2xl overflow-hidden border border-gray-200 bg-gray-100 group">
          ...
          {file.existing && (
            <span className="absolute top-2 right-2 text-[9px] bg-sky-500 text-white px-1.5 py-0.5 rounded-md font-bold">Saved</span>
          )}
          ...
          <div className="absolute bottom-0 left-0 right-0 px-3 py-2 bg-gradient-to-t from-black/60 to-transparent">
            <p className="text-white text-xs font-medium truncate">{file.file?.name || "Existing image"}</p>
          </div>
        </div>
      )}
    </div>
...
const ImageManager = ({ images, onAdd, onRemove, label = "Upload images", hint }) => {
  const ref = useRef();
    ...
    <div className="space-y-3">
      ...
      {images.length > 0 && (
        <div className="grid grid-cols-4 gap-2">
          {images.map((f, i) => (
            <div key={f.id || i} className="relative group rounded-xl overflow-hidden border border-gray-200 bg-gray-100 aspect-square">
              <img src={f.preview} alt="" className="w-full h-full object-cover" />
              {f.existing && <span className="absolute top-1 right-1 text-[9px] bg-sky-500 text-white px-1.5 py-0.5 rounded-md font-bold">Saved</span>}
              {i === 0 && !f.existing && <span className="absolute top-1 left-1 text-[9px] bg-emerald-500 text-white px-1.5 py-0.5 rounded-md font-bold">Main</span>}
              {i === 0 && f.existing  && <span className="absolute top-1 left-1 text-[9px] bg-emerald-500 text-white px-1.5 py-0.5 rounded-md font-bold">Main</span>}
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                <button type="button" onClick={() => onRemove(f.id ?? i)} className="p-1.5 bg-red-500 rounded-lg text-white">
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>
            </div>
...
// ─── Variant Editor ───────────────────────────────────────────────────────────
const VariantEditor = ({ variant, index, variantImages, onVariantChange, onVariantImageAdd, onVariantImageRemove, onRemove, isOnly }) => {
  const setV = (path, val) => {
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-gray-100">
        <div className="flex items-center gap-2">
          <span className="w-6 h-6 rounded-lg bg-amber-100 text-amber-700 text-xs font-bold flex items-center justify-center">{index + 1}</span>
          <span className="text-sm font-semibold text-gray-700">
            Variant {index + 1} {variant.sku && <span className="text-gray-400 font-normal">— {variant.sku}</span>}
          </span>
        </div>
        <div className="flex items-center gap-3">
          <Toggle checked={!!variant.isDefault} onChange={(v) => setV("isDefault", v)} label="Default" />
          {!isOnly && (
            <button type="button" onClick={onRemove} className="p-1.5 rounded-lg text-red-400 hover:bg-red-50 transition">
              <Trash2 className="w-4 h-4" />
            </button>
          )}
        </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="SKU" required>
            <Input placeholder="PROD-50ML" value={variant.sku || ""} onChange={(e) => setV("sku", e.target.value.toUpperCase())} />
          </Field>
          <Field label="Barcode">
            <Input placeholder="8901234567890" value={variant.barcode || ""} onChange={(e) => setV("barcode", e.target.value)} />
          </Field>
        </div>

        {/* Attributes */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Field label="Size">    <Input placeholder="50ml"      value={variant.attributes?.size   || ""} onChange={(e) => setV("attributes.size",   e.target.value)} /></Field>
          <Field label="Shade">   <Input placeholder="Rose Beige" value={variant.attributes?.shade  || ""} onChange={(e) => setV("attributes.shade",  e.target.value)} /></Field>
          <Field label="Scent">   <Input placeholder="Lavender"   value={variant.attributes?.scent  || ""} onChange={(e) => setV("attributes.scent",  e.target.value)} /></Field>
          <Field label="Pack Of"> <Input type="number" min="1" placeholder="1" onWheel={(e) => e.target.blur()} value={variant.attributes?.packOf || ""} onChange={(e) => setV("attributes.packOf", e.target.value)} /></Field>
        </div>

        {/* Pricing */}
        <div className="grid grid-cols-2 gap-3">
          <Field label="MRP (₹)" required>
            <Input type="number" min="0" placeholder="599" onWheel={(e) => e.target.blur()} value={variant.price?.mrp ?? ""} onChange={(e) => setV("price.mrp", e.target.value)} />
          </Field>
          <Field label="Selling Price (₹)" required>
            <Input type="number" min="0" placeholder="399" onWheel={(e) => e.target.blur()} value={variant.price?.sellingPrice ?? ""} onChange={(e) => setV("price.sellingPrice", e.target.value)} />
          </Field>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Field label="Stock Qty">
            <Input type="number" min="0" placeholder="100" onWheel={(e) => e.target.blur()} value={variant.stock?.quantity ?? ""} onChange={(e) => setV("stock.quantity", e.target.value)} />
          </Field>
          <Field label="Low Stock Alert">
            <Input type="number" min="0" placeholder="10" onWheel={(e) => e.target.blur()} value={variant.stock?.lowStockAlert ?? ""} onChange={(e) => setV("stock.lowStockAlert", e.target.value)} />
          </Field>
          {/* <Field label="Net Weight">
            <Input type="number" min="0" placeholder="50" onWheel={(e) => e.target.blur()} value={variant.weight?.value ?? ""} onChange={(e) => setV("weight.value", e.target.value)} />
          </Field> */}
          {/* <Field label="Unit">
            <SelectEl value={variant.weight?.unit || "ml"} onChange={(e) => setV("weight.unit", e.target.value)}>
              {WEIGHT_UNITS.map((u) => <option key={u}>{u}</option>)}
            </SelectEl>
          </Field> */}
        </div>

        <Toggle checked={!!variant.isActive} onChange={(v) => setV("isActive", v)} label="Variant Active" />

        {/* ── Variant Images ── */}
        <div className="pt-3 border-t border-gray-200">
          <p className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">
            Variant Images <span className="text-gray-300 font-normal">(optional — overrides product images)</span>
          </p>
          <ImageManager
...
const IngredientRow = ({ ing, onChange, onRemove }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    <div className="col-span-3"><Field label="INCI">   <Input placeholder="INCI"       value={ing.inci    || ""} onChange={(e) => onChange({ ...ing, inci:    e.target.value })} /></Field></div>
    ...
    <div className="col-span-1"><Field label="%">      <Input type="number" min="0" max="100" placeholder="5" onWheel={(e) => e.target.blur()} value={ing.percentage ?? ""} onChange={(e) => onChange({ ...ing, percentage: e.target.value })} /></Field></div>
...
// ─── Edit Product Form ────────────────────────────────────────────────────────
function EditProductForm({ product, onClose, fetchProducts, fetchCategories, fetchStats }) {
  const [submitted,  setSubmitted]  = useState(false);
  const [loading,    setLoading]    = useState(false);

  const [form, setForm] = useState({
    name: "", brand: "", category: "", subCategory: "",
    tag: null, shortDescription: "", description: "",
    highlights: [""],
    isFeatured: false, isBestseller: false, isNewArrival: false,
    isActive: true, isFreeShipping: false,
    hsn: "", taxRate: 18,
    skincareDetails: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const cleanedVariants = variants.map(({ _id, createdAt, updatedAt, images: _vi, ...v }) => ({
        ...v,
        ...
        weight: { value: Number(v.weight?.value || 0), unit: v.weight?.unit || "ml" },
      ...
      fd.append("isFreeShipping",   String(form.isFreeShipping));
      ...
      const existingPids = images.filter((img) => img.existing && img.public_id).map((img) => img.public_id);
      fd.append("existingImages", JSON.stringify(existingPids));

      // New product images
      images.forEach((img) => { if (!img.existing && img.file) fd.append("images", img.file); });
...
variantImageGroups.forEach((group, variantIndex) => {
  ...
  fd.append(`existingVariantImages_${variantIndex}`, JSON.stringify(existingVariantPids));
  ...
  group.forEach((img) => {
    if (!img.existing && img.file) {
      ...
      fd.append(`variantImageAlt_${variantIndex}_${img.file.name}`, img.altText || "");
    }
  });
});

      const response = await fetch(`${BASE_URL}/admin/products/${product._id}`, {
        method:  "PUT",
  ...
  const emptyVariant = () => ({
    id: Date.now() + Math.random(),
    sku: "", attributes: { size: "", shade: "", scent: "", packOf: "" },
    price: { mrp: "", sellingPrice: "" },
    stock: { quantity: "", lowStockAlert: 10 },
    weight: { value: "", unit: "ml" },
    barcode: "", isDefault: false, isActive: true,
  });
      ...
      <SectionCard icon={Info} title="Basic Information" color="emerald">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Category" required>
            <Input placeholder="Face Serum" value={form.category} onChange={(e) => set("category", e.target.value)} />
          </Field>
        ...
        {/* Tag selector */}
        <Field label="Product Tag" hint="Pick one — shown as a badge on the product card" required>
          <div className="flex flex-wrap gap-2">
            {TAGS.map((opt) => {
            ...
            {form.tag && (
              <button type="button" onClick={() => set("tag", null)}
...
>
  ...
  <Field
    ...
    hint="First image = main display image"
  >
    <ImageManager
      images={images}
 ...
 <Field
  ...
  hint="First video = main display video"
>
  <VideoManager
    videos={videos}
  ...
  <div className="p-4 rounded-2xl bg-emerald-50/60 border border-emerald-100 space-y-3 mt-6">
    ...
    {categoryImage?.existing && (
        ...
        <AnimatePresence>
          {variants.map((v, i) => (
            <VariantEditor
              ...
              onVariantChange={(updated) => {
                const arr = [...variants];
                if (updated.isDefault) arr.forEach((x) => (x.isDefault = false));
                arr[i] = updated;
                setVariants(arr);
              }}
              ...
              onVariantImageRemove={(idOrIdx) => {
                setVariantImageGroups((prev) => {
                  const g = [...prev];
                  g[i] = (g[i] || []).filter((f, fi) => (f.id ?? fi) !== idOrIdx);
                  return g;
                });
              }}
              onRemove={() => {
                setVariants(variants.filter((_, idx) => idx !== i));
                setVariantImageGroups((prev) => prev.filter((_, idx) => idx !== i));
              }}
            />
          ))}
        </AnimatePresence>
      ...
      <SectionCard icon={Leaf} title="Skincare Details" color="emerald" defaultOpen={false}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Shelf Life (months)"><Input type="number" placeholder="24" onWheel={(e) => e.target.blur()} value={form.skincareDetails.shelfLife.months}    onChange={(e) => set("skincareDetails.shelfLife.months",    e.target.value)} /></Field>
          <Field label="PAO (months)">       <Input type="number" placeholder="12" onWheel={(e) => e.target.blur()} value={form.skincareDetails.shelfLife.paoMonths} onChange={(e) => set("skincareDetails.shelfLife.paoMonths", e.target.value)} /></Field>
        </div>
        ...
        <Field label="Free From"      hint="Comma-separated"><Input placeholder="Mineral Oil, SLS"      value={form.skincareDetails.madeWithoutList} onChange={(e) => set("skincareDetails.madeWithoutList", e.target.value)} /></Field>
      </SectionCard>
      ...
      <SectionCard icon={Leaf} title="Ingredients (INCI List)" color="rose" defaultOpen={false}>
        <AnimatePresence>
          {ingredients.map((ing, idx) => (
            <IngredientRow key={ing._id || ing.id || idx} ing={ing}
              onChange={(updated) => setIngredients(ingredients.map((x, i) => i === idx ? updated : x))}
              onRemove={() => setIngredients(ingredients.filter((_, i) => i !== idx))} />
          ))}
        </AnimatePresence>
        {ingredients.length === 0 && <p className="text-sm text-gray-400 text-center py-4">No ingredients yet.</p>}
      ...
      <SectionCard icon={Box} title="Packaging, Shipping & Tax" color="slate" defaultOpen={false}>
        <div className="grid grid-cols-3 gap-4">
          <Field label="Package Type">
            <SelectEl value={form.packaging.type} onChange={(e) => set("packaging.type", e.target.value)}>
        ...
        <div className="grid grid-cols-2 gap-4 pt-2 border-t border-gray-100">
          ...
          <Field label="GST Tax Rate (%)">
            <SelectEl value={form.taxRate} onChange={(e) => set("taxRate", Number(e.target.value))}>
      ...
      <div className="flex items-center justify-end gap-3 sticky bottom-0 bg-white border-t border-gray-100 px-6 py-4 rounded-b-2xl">
        ...
        <button type="submit" disabled={loading}
          ...
          {loading   ? <><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" /> Saving…</>
          : submitted ? <><Check className="w-4 h-4" /> Updated!</>
...
// ─── Modal Wrapper ────────────────────────────────────────────────────────────
export default function EditProduct({ open, onClose, product, fetchProducts, fetchCategories,  fetchStats }) {
